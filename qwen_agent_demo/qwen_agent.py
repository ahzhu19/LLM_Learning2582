...
def agent_execute(query,chat_history = []):
    global tools, tool_names,tool_docs,prompt_tpl,llm,tokenizer
    ...
    while True:
        ...
        prompt=prompt_tpl.format(today=today,chat_history=history,
                                 tool_descs=tool_docs,tool_names=tool_names,
...
def agent_execute_with_retry(query,chat_history,retry_times=3):
    for i in range(retry_times):
        success,result,new_chat_history=agent_execute(query,chat_history)
        if success:
            return True,result,new_chat_history
    return False,None,chat_history
...
while True:
    query=input('query:')
    success,result,my_history=agent_execute_with_retry(query,chat_history=my_history) #TODO
